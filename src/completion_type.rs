...
impl CircularListHelper {
    ...
    fn build_and_render_block<H: Helper, C: Candidate>(
        ...
        // To avoid replacing on first tab
        if replace_line {
        ...
        let (mut circular_list, num_rows_including_linewraps) = circular_list(
            &ListLayout {
                ...
                index: idx,
                highlight_selection: replace_line,
            },
            &candidates,
        );
    ...
    fn render_block<H: Helper>(&mut self, s: &mut State<'_, '_, H>) -> Result<()> {
        ...
        s.out.write_and_flush(&self.circular_list[..])?;
    ...
    for row in begin..end {
        ...
        for col in 0..num_cols {
            ...
            if idx < candidates.len() {
                ...
                let colored = if list_layout.highlight_selection && list_layout.index == idx {
                    candidate.black().on_white().to_string()
                } else {
                    candidate.to_string()
                };
                circ_list.push_str(&colored);
...
pub(crate) fn circular_completion_list_loop<H: Helper, He: Helper>(
    ...
    // Show longest common prefix immediately
    ...
    // Don't display anything if there is one candidate
    if candidates.len() == 1 {
        return Ok(None);
    }
    ...
    block_at_the_end.build_and_render_block(s, &candidates, idx, start, replace_line)?;
    ...
    'reload_completer: loop {
        ...
        loop {
            ...
            if idx < candidates.len() {
                replace_line = true;
                block_at_the_end.build_and_render_block(
                    s,
                    &candidates,
                    idx,
                    start,
            ...
            match cmd {
                Cmd::Complete => {
                    idx = (idx + 1) % (candidates.len()); // Circular
                    if idx == candidates.len() {
                        s.out.beep()?;
                    }
                }
                ...
                Cmd::AcceptOrInsertLine {
                    ...
                    // AcceptOrInsertLine confirms an element from the displayed CircularList
